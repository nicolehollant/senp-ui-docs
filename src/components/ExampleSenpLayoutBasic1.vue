<template>
  <div class="relative flex flex-col pt-2 w-full min-h-96 not-prose">
    <DocsTab :options="options" :classes="{ options: { extend: 'text-sm' }, optionsWrapper: { extend: 'pb-2' } }" chips>
      <template #default="{ active }: { active: (typeof options)[number] }">
        <div v-if="active === 'base'" class="border-2 border-neutral-500/60 bg-green-800 h-[500px] overflow-auto">
          <SenpLayoutBasic
            :classes="{
              header: {
                extend: 'bg-green-900 shadow',
              },
              content: {
                extend: 'bg-zinc-900/90 !grid grid-cols-2 ',
              },
              footer: {
                extend: 'bg-zinc-700/80',
              },
            }"
          >
            <template #header>
              <h1 class="text-xl font-serif tracking-wider text-green-200">🪴 Plant.co</h1>
            </template>
            <template #content>
              <div
                v-for="plant in plants"
                :key="plant.uri"
                class="rounded-xl overflow-hidden flex flex-col gap-2 bg-green-800/40 shadow"
              >
                <img :src="plant.uri" class="w-full h-32 object-cover" />
                <div class="px-2 grid gap-1 pb-2">
                  <p class="text-green-300 font-serif">{{ plant.title }}</p>
                  <p class="text-xs font-serif text-green-300/80">
            ...
            <template #footer>
              <p>© Plant.co 2022, all rights reserved</p>
            </template>
          </SenpLayoutBasic>
        </div>
        <div v-if="active === 'fixed_head'" class="border-2 border-neutral-500/60 bg-green-800 h-[500px] overflow-auto">
          <SenpLayoutBasic
            fixed-header
            :classes="{
              header: {
                extend: 'bg-green-900/80 backdrop-blur shadow',
              },
              content: {
                extend: 'bg-zinc-900/90 !grid grid-cols-2 ',
              },
        ...
        <div v-if="active === 'fixed_foot'" class="border-2 border-neutral-500/60 bg-green-800 h-[500px] overflow-auto">
          <SenpLayoutBasic
            fixed-footer
            :classes="{
              header: {
              ...
              footer: {
                extend: 'bg-zinc-700/80 backdrop-blur',
              },
            }"
          >
            <template #header>
        ...
        <div v-if="active === 'fixed_both'" class="border-2 border-neutral-500/60 bg-green-800 h-[500px] overflow-auto">
          <SenpLayoutBasic
            fixed-header
            fixed-footer
            :classes="{
        ...
        <div
          v-if="active === 'sidebar_left'"
          class="border-2 border-neutral-500/60 bg-green-800 h-[500px] overflow-auto"
        >
          <SenpLayoutBasic
            sidebar-left
            :classes="{
          ...
          >
            ...
            <template #sidebar-left>
              <div
                class="h-16 sticky top-0 bg-green-800/50 border-b backdrop-blur border-green-900 -mx-2 px-2 flex items-center shrink-0"
              >
                Articles
              </div>
              <nav class="grid gap-3">
                <li v-for="plant in plants" class="text-xs list-none text-green-100/70">
                  {{ plant.title }}
                </li>
              </nav>
            </template>
        ...
        >
          <SenpLayoutBasic
            ...
            :classes="{
              ...
              sidebarRight: {
                extend: '!max-w-[120px] pb-8 !min-w-0 bg-neutral-800/50',
              },
            }"
          >
            <template #header>
            ...
            <template #sidebar-right>
              <div class="h-16 sticky top-0 -mx-2 px-2 flex items-center shrink-0">About</div>
              <p class="text-xs">Plant.co is a fake site to use as an example for these layouts...</p>
            </template>
            <template #content>
              <div
                v-for="plant in plants"
...
const plants = [
  ...
  {
    title: 'How to get a green thumb',
    description: '',
    timestamp: new Date(Date.now() - Math.floor(Math.random() * 1000000000)),
...
const options = Object.keys(values) as Array<keyof typeof values>
</script>
